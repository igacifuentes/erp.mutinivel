...
class dashboard extends CI_Controller {
	...
	private function requisitoMesesPremio($id_afiliado, $meses, $codificados ,$id_premio){
		$fechaActual = date('Y-m-d');
		$premio = $this->modelo_premios->consultarPremioObtenido($id_premio - 1, $id_afiliado);
		$siCumple = true;
			
		if(isset($premio[0]->id)){
			$fechaValida = strtotime ( '-'.$meses.' month' , strtotime ( $fechaActual ) ) ;
			$fechaPremio = strtotime($premio[0]->fecha);
			
			if($fechaPremio > $fechaValida && $premio[0]->id_premio != 1){
				return false;
			}
	...
	function index() {
		...
		$premios = $this->modelo_premios->consultarPremiosObtenido($id);
