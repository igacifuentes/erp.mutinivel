				...
				<div class="row">
					<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
						...
						<a class="backHome" href="/bo"><i class="fa fa-home"></i> Menu</a>
							<span>>
								<a href="/bo/oficinaVirtual/"> Oficina Virtual</a> > <a href="/bo/oficinaVirtual/videos"> Videos</a> > <a href="/bo/videos/listar_videos"> Listar </a> > Listar niveles
							</span>
						</h1>
					</div>
				</div>
													...
													<tr>
														...
														<?php 
															foreach ($mercancia as $Mercancia) {
																if ($Mercancia->id==$Nivel->id_Nivel){

																	foreach ($producto as $Producto) {
																		if ($Producto->id==$Mercancia->sku and $Mercancia->id_tipo_mercancia==1){
																			echo $Producto->nombre;
																		}
																		
																	}
																	foreach ($servicio as $Servicio) {
																		if ($Servicio->id==$Mercancia->sku and $Mercancia->id_tipo_mercancia==2){
																			echo $Servicio->concepto;
																		}
																		
																	}
																	foreach ($paquete_inscripcion as $Paquete_inscripcion) {
																		if ($Paquete_inscripcion->id_paquete==$Mercancia->sku and $Mercancia->id_tipo_mercancia==3){
																			echo $Paquete_inscripcion->nombre;
																		}
																		
																	}
																	foreach ($combinado as $Combinado) {
																		if ($Combinado->id==$Mercancia->sku and $Mercancia->id_tipo_mercancia==4){
																			echo $Combinado->nombre;
																		}
																		
																	}		

																	foreach ($membresia as $Membresia) {
																		if ($Membresia->id==$Mercancia->sku and $Mercancia->id_tipo_mercancia==4){
																			echo $Membresia->nombre;
																		}
														...
														<td>
															...
															<?php } else {?>
																<a style="cursor: pointer;" title="Activar" onclick="estado('ACT','<?php echo $Nivel->id_Nivel; ?>')" class="txt-color-green"><i class="fa fa-square-o fa-3x"></i></a>
															<?php } ?>
														</td>
													</tr>
												<?}?>
...
function estado(estatus, id)
{
	...
	$.ajax({
		type: "POST",
		url: "/bo/videos/cambiar_estado_nivel",
		data: {
			id:id, 
			estado: estatus
		},
